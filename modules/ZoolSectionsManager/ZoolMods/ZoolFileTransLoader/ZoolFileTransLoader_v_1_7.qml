...
Rectangle {
    ...
    function searchBodieDateFronLong(numAstro, g, ai, mi, di, af, mf, df, tol){
        ...
        //searchAspsBodieFromLong(int di, int mi, int ai, int indexBodie1, double degBodie1, int d2, int m2, int a2, double jdPorcIncrement)
        //let jasps=swe.searchAspsBodieFromLong(di, mi, ai, numAstro, g, df, mf, af, tol, bb.bsel2)
        let idSwissEphBodieEstatico=zm.aIndexSweBodies[bb.bsel2]
        if(idSwissEphBodieEstatico===10)idSwissEphBodieEstatico=-10

        let idSwissEphMovil=bb.bsel2
        if(idSwissEphMovil===10)idSwissEphMovil=-10
        let jasps=swe.searchAspsBodieFromLong(di, mi, ai, numAstro, g, df, mf, af, tol, [idSwissEphMovil])
        revSearchBodieDateFronLongResult(jasps)
        return
        let d = new Date(Date.now())
        let ms=d.getTime()
        let c='import QtQuick 2.0\n'
    ...
    function revSearchBodieDateFronLongResult(j){
        ...
        if(j.isData){
            ...
            if(j.gb>gred && j.tol < 0.005){
                console.log('Tránsito: Calculando')
                ...
                if(!tl.isDateInList(j)){
                    tl.addItem(j)
                    console.log('Tránsito: Fecha obtenida')
                }
