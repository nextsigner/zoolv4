...
Menu {
    ...
    property int fs: app.fs * 0.75
    property real scaleExpandWidth: 0.6 // Ajustado para dar un poco más de aire


    // Ancho calculado dinámicamente
    width: Math.max(implicitWidth, calculatedWidth)
    property real calculatedWidth: 200

    // Estilo de fondo del Menú completo
    background: Rectangle {
    ...
    // Función para calcular el ancho óptimo basado en el texto más largo
    function updateWidth() {
        let maxChars = 0
        for (let i = 0; i < r.count; i++) {
            let itemText = ""
            let action = r.actionAt(i)
            let subMenu = r.menuAt(i)

            if (action) itemText = action.text
            else if (subMenu) itemText = subMenu.title

            if (itemText.length > maxChars) {
                maxChars = itemText.length
            }
        }
        // Cálculo basado en el tamaño de fuente y un factor de escala
        r.calculatedWidth = maxChars * (r.fs * r.scaleExpandWidth)
    }

    // Se ejecuta justo antes de mostrarse
    ...
    delegate: MenuItem {
        ...
        // El item ocupa todo el ancho del menú
        width: parent.width
        implicitHeight: r.fs * 1.6

        // Soporte para flecha de submenú (opcional pero recomendado)
        arrow: Canvas {
            ...
            onPaint: {
                ...
                ctx.lineTo(0, height)
                ctx.closePath()
                ctx.fillStyle = menuItem.highlighted ? apps.fontColor : apps.backgroundColor
                ctx.fill()
            }
        }
        ...
        background: Rectangle {
            color: menuItem.highlighted ? apps.backgroundColor : "transparent"
        }
    }
}
