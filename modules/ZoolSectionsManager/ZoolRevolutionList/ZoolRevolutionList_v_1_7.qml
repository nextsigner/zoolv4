...
Rectangle {
    ...
    Component{
        ...
        Rectangle{
            ...
            onIsChanged:{
                iconoSigno.source=r.folderImgs+'/signs/'+is+'.svg'
            }
            //Behavior on height{enabled: apps.enableFullAnimation;NumberAnimation{duration: app.msDesDuration}}
            ...
            Component.onCompleted: {
                let p=zfdm.getJsonAbsParams(false)
                //log.lv('RS params:::'+JSON.stringify(p, null, 2))
                //log.lv('RS Grado Sol:::'+zm.currentJson.pc.c0.gdec)
                //log.lv('RS:::'+JSON.stringify(json))
                let ar = swe.getSolarReturn(zm.currentJson.pc.c0.gdec, json.a, p.m, p.d, p.gmt)
                //log.lv('RS Array:::'+ar)

                let jsonHouses=JSON.parse(swe.getHousesPos(ar[0], ar[1], ar[2], ar[3], ar[4], p.gmt, p.lon, p.lat, p.alt, p.hsys))
                //log.lv('RS ASC:::'+JSON.stringify(jsonHouses.ph.h1, null, 2))
                //log.lv('RS h1.gdec:::'+jsonHouses.ph.h1.gdec)
